...
function buildSignal({ symbol, candles, ticker, btcRegime, settings }) {
  ...
  // Entry: retest of the level behind the move. Stop: beyond that level with an ATR cushion so
  // it is not sitting exactly where every other stop is. Target: the next opposing level — NOT
  // a fixed multiple. If no opposing level exists yet, fall back to 2.5x the stop distance so
  // the plan still has a target, but a real level always wins when one is there. RR_BOUNDS
  // (default 2.0-6.0) is what actually constrains the ratio; this is not a fixed 1:2 or 1:3 system.
  let entry;
  let sl;
  let tp;
  ...
  if (isBuy) {
    ...
    sl = support - a * 0.5;
    tp = resistance != null && resistance > entry ? resistance - a * 0.1 : entry + Math.abs(entry - sl) * 2.5;
  } else {
    if (resistance == null) return { ok: false, reason: 'NO_RESISTANCE_LEVEL' };
    entry = Math.max(price, resistance - a * 0.25);
    sl = resistance + a * 0.5;
    tp = support != null && support < entry ? support + a * 0.1 : entry - Math.abs(entry - sl) * 2.5;
  }
